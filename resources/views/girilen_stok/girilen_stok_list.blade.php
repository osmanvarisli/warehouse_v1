...
<div class="row">
    <div class="col-lg-12">
        <h1 class="page-header">Giriş Listesi</h1>
    </div>
    <!-- /.col-lg-12 -->
</div>
...
<table id="basic" class="table table-bordered table-striped">
    ...
    <tr>
        <td>{{ $stok->depo_ad }}</td>
        <td>{{ $stok->user }}</td>
        <td>{{ $stok->giris_tarihi }}</td>
        <td>{{ $stok->aciklama }}</td>
        <td>
            <form method="post" action="{{route('girilen_stok_delete', ['id' => $stok->id]) }}">
            @csrf
        ...
        <td><a  href="{{ route('girilen_stok_duzenle', ['id' => $stok->id]) }}" class="btn btn-default">Düzenle</a></td>
